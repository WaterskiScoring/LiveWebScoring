Public Class TROandBrNCW
        ...
        If Not IsPostBack Then
            ...
            Try
                ...
                If Not Regex.IsMatch(sFormatCode, "^[NCWROL]*$") Then
                    ...
                    Response.Redirect("~/default.aspx?EM=" & sEM)
                End If
                If sFormatCode <> "NCWRO" Then  'Make sure this is the correct display form for the format
                    sEM = "Invalid Request.  You may try again."
                    Session.Abandon()
                    Response.Redirect("~/default.aspx?EM=" & sEM)
                End If
            ...
            If sMsg <> "Success" Then
                ...
                Exit Sub
            End If
            'OnWater display is not affected by event choice.
            'If no skiers found in last 10 minutes ModDataAccess3.XXX returns "No Skiers Found" which hides the display
    ...
    Private Sub UpdateRO()
        ...
        If sDivisionCodePkd = "" Then
            sDivisionCodePkd = "ALL"
        End If
        Dim sEventCodePkd As String = DDL_EventsPkd.SelectedValue
        Dim sRndsPkd As String = "1"    'NCWSA always 1 round DDL_PkRnd.SelectedValue is not available
        ...
        Dim sDisplayMetric As Int16 = 0
        Try
            sDisplayMetric = CInt(HF_DisplayMetric.Value)
        Catch ex As Exception
            'IF value is not numeric - leave sDisplayMetric = 0
        End Try
        lbl_Errors.Text = ""
        lbl_Errors.Visible = False
        Dim sCurrentCount As Int16 = 0
        'OnWater display is not affected by event choice.
        ...
        If (sEventCodePkd = "J" Or sEventCodePkd = "A") And sJumpRounds > 0 Then
            ...
            End If
        End If
        If sCurrentCount = 0 Then
            'display onwater using collapse or canvass
        Else
            'hide onwater using collapse or canvass
        End If
        Select Case sEventCodePkd
            Case "0"
                ColumnScores.InnerHtml = "Please Select an Event"
            Case "A"
                ColumnScores.InnerHtml = ModDataAccessTeams.ScoresXRunOrdHorizNCW(sSanctionID, sYrPkd, sTournName, "S", sSlalomrounds, sTrickRounds, sJumpRounds, sDivisionCodePkd, sRndsPkd, sDisplayMetric)
                ColumnScores.InnerHtml += ModDataAccessTeams.ScoresXRunOrdHorizNCW(sSanctionID, sYrPkd, sTournName, "T", sSlalomrounds, sTrickRounds, sJumpRounds, sDivisionCodePkd, sRndsPkd, sDisplayMetric)
                ColumnScores.InnerHtml += ModDataAccessTeams.ScoresXRunOrdHorizNCW(sSanctionID, sYrPkd, sTournName, "J", sSlalomrounds, sTrickRounds, sJumpRounds, sDivisionCodePkd, sRndsPkd, sDisplayMetric)
            Case Else
                ColumnScores.InnerHtml = ModDataAccessTeams.ScoresXRunOrdHorizNCW(sSanctionID, sYrPkd, sTournName, sEventCodePkd, sSlalomrounds, sTrickRounds, sJumpRounds, sDivisionCodePkd, sRndsPkd, sDisplayMetric)
        End Select
    End Sub
    ...
    Private Sub DDL_EventsPkd_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DDL_EventsPkd.SelectedIndexChanged
        ...
        Dim sTrickRounds As String = HF_TrickRnds.Value
        Dim sSanctionID As String = HF_SanctionID.Value
        ModDataAccess.LoadDvList(sSanctionID, sEventCodePkd, sDivisionCodePkd, sRndsPkd, DDL_DvPkd, sSlalomRounds, sTrickRounds, sJumpRounds, DDL_PkRnd)
        UpdateRO()
    End Sub
