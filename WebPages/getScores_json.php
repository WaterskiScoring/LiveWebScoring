...
if (checkReqVars()) {
	...
	if ( $_SESSION['skiEvent'] == 'Recent') {
		...
		$QueryCmd = "SELECT tri.SkierName, ssi.MemberId, ssi.AgeGroup, ssi.Score as EventScore, er.TeamCode, Round"
			...
			. "UNION "
			. "SELECT tri.SkierName, ssi.MemberId, ssi.AgeGroup, ssi.Score as EventScore, er.TeamCode, Round"
			...
			. "WHERE ssi.SanctionID='" .  $thisSanc . "' "
			. "UNION "
			. "SELECT tri.SkierName, ssi.MemberId, ssi.AgeGroup, ssi.ScoreFeet as EventScore, er.TeamCode, Round"
		...
		$QueryCmd = "SELECT TR.SkierName, TR.MemberId, TR.AgeGroup, 'Overall' as Event "
		...
		if ($thisSkiEvent == "Slalom") {
			$QueryCmd = "SELECT tri.SkierName, ssi.MemberId, ssi.AgeGroup, Round"
	...
	if ( $curRowCount != 0 ) {
		...
		if ( $_SESSION['skiEvent'] == 'Overall') {
			while ($ScoresRow = mysql_fetch_assoc($QueryResult)) {
				...
				echo "<li>\r\n";
				echo "<a href='wfwShowScoreRecap.php?MemberId=" . $ScoresRow['MemberId'] . "&SkierName=" . $ScoresRow['SkierName'] . "&SkierRound=" . $ScoresRow['Round'] . "&SkierEvent=" . $ScoresRow['Event'] . "' "
				. "data-rel='dialog' data-transition='pop'>" . $ScoresRow['SkierName']
			...
			while ($ScoresRow = mysql_fetch_assoc($QueryResult)) {
				if ($thisDivision == "Recent" || $_SESSION['skiEvent'] == 'Recent' ) {
					...
					if ( $ScoresRow['AgeGroup'] != $prevGroup ) {
						...
						$CountQueryResult = mysql_query($QueryCmd) or die (mysql_error());
						$curRowCount = mysql_num_rows($CountQueryResult);
						if ( $curRowCount != 0 ) {
							$CountRow = mysql_fetch_assoc($CountQueryResult);
						...
						mysql_free_result($CountQueryResult);
				...
				echo "<li>\r\n";
				echo "<a href='wfwShowScoreRecap.php?MemberId=" . $ScoresRow['MemberId']
					. "&SkierName=" . $ScoresRow['SkierName']
