...
**************************************************************************** */
CREATE PROCEDURE dbo.PrTrickScoresByRunOrder @InSanctionId AS varchar(6), @InRound as Char(1), @InDiv as varchar(3) = 'All'
AS
BEGIN
	...
	SET @curSqlStmt = 'SELECT TR.SkierName, TR.SanctionId, TR.MemberId, TR.SkiYearAge, TR.AgeGroup, TR.AgeGroup as Div, TR.Gender, TR.City, TR.State, TR.Federation'
		...
		+ 'LEFT OUTER JOIN TrickScore AS SS ON SS.MemberId = TR.MemberId AND SS.SanctionId = TR.SanctionId AND SS.AgeGroup = TR.AgeGroup AND (' + @InRound + ' = 0 OR Round = ' + @InRound + ') '
