...
**************************************************************************** */
IF OBJECT_ID ('dbo.PrTrickScoresByRunOrder') IS NOT NULL
	DROP PROCEDURE dbo.PrTrickScoresByRunOrder
GO
...
/* ****************************************************************************
Object:			StoredProcedure dbo.PrTrickScoresByRunOrder
Author:			David Allen
Create date: 	January 31, 2024
Description:	Retrieve tournament Trick scores sequenced by running order
Value Values: 	Event (Trick, Trick, Jump)
				Div (All or any valid age group code 

**************************************************************************** */
CREATE PROCEDURE dbo.PrTrickScoresByRunOrder @InSanctionId AS varchar(6), @InRound as Char(1), @InDiv as varchar(3) = 'All', @InGroup as varchar(3) = 'All'
AS
BEGIN
	...
	SET @curRoundFilter = ''
	IF @InRound > 0 BEGIN
		SET @curCustomOrderCount = (Select Count(*) as CustomRowCount From EventRunOrder Where SanctionId = @InSanctionId AND Event = 'Trick' AND Round = @InRound )
	END
	...
	IF @InRound = 0 OR @curCustomOrderCount = 0 BEGIN
		SET @curPropKey = 'RunningOrderSortTrick';
	...
	ELSE BEGIN
		...
		SET @curSqlStmt = 'SELECT TR.SkierName, TR.SanctionId, TR.MemberId, TR.SkiYearAge, TR.AgeGroup, TR.AgeGroup as Div, TR.Gender, TR.City, TR.State, TR.Federation'
			...
			+ 'LEFT OUTER JOIN TrickScore AS SS ON SS.MemberId = TR.MemberId AND SS.SanctionId = TR.SanctionId AND SS.AgeGroup = TR.AgeGroup AND SS.Round = RO.Round '
			+ 'LEFT OUTER JOIN TrickVideo AS TV ON TV.SanctionId = TR.SanctionId AND TV.MemberId = TR.MemberId AND TV.AgeGroup = TR.AgeGroup AND TV.Round = SS.Round '
			+ 'LEFT OUTER JOIN DivOrder D ON D.SanctionId = ER.SanctionId AND D.AgeGroup = ER.AgeGroup AND D.Event = ER.Event '
			+ 'Where TR.SanctionId = ''' + @InSanctionId + ''' '
			+ @curDivFilter
...
GRANT EXECUTE ON dbo.PrTrickScoresByRunOrder TO PUBLIC
GO
